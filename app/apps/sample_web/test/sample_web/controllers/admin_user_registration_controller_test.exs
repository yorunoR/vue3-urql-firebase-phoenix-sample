defmodule SampleWeb.AdminUserRegistrationControllerTest do
  ...
  describe "POST /admin_users/register" do
    @tag :capture_log
    test "creates account and logs the admin_user in", %{conn: conn} do
      email = unique_admin_user_email()

      conn =
        post(conn, Routes.admin_user_registration_path(conn, :create), %{
          "admin_user" => valid_admin_user_attributes(email: email)
        })
    ...
    test "render errors for invalid data", %{conn: conn} do
      conn =
        post(conn, Routes.admin_user_registration_path(conn, :create), %{
          "admin_user" => %{"email" => "with spaces", "password" => "too short"}
        })
